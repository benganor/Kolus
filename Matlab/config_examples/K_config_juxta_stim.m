...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CURRENT CONFIG: K_config_multiunit_stim.m
...
%Last argument onl  y for analog input: range of voltage,  digital output: type of signal
...
%NOTE on analog LASER: minimum is 1.1v, max is 6v
%Name of channel - 1st 4 letters determine variable in saved data
daq_channels = { ...
{'microphone' , 'ai0', 'analog input', 250e3, [-1,1]}, ...
{'stimulus input', 'ai2', 'analog input', 2500, [-.2,.2]} , ...
{'piezo input', 'ai3', 'analog input', 2500, [-.2,.2]} , ...
{'neuron', 'ai5', 'analog input', 50e3, [-10,10]} , ...
{'stimulus output', 'ao0', 'analog output', nan, [-10 10], 'stimuli'}, ...
{'stimulus output', 'ao1', 'analog output', nan, [-10 10], 'piezo'}, ...
...
{'Microphone', 'sound'}, ...
{'Neuron', 'time series'}, ...
{'Stimulus', 'digital', 'green'}, ...
{'Stimulus', 'digital', 'blue'}
};
pp.ylims = [[0 125]; [-5 5]];;
...
tag.Experiment = 'OS'; %name of experimental setup - optical stimulation
...
% 'continuous' pulse or 
% 'train' of pulses or 
% 'cosine' - for ephys/neuropix
%times should be in increments of 1/window_FPS, ie .1 seconds
tag.param_stim.Type = 'continuous'; % 'train'
tag.param_stim.Duration_pre = 2; %seconds
tag.param_stim.Duration = 3; %seconds
tag.param_stim.Duration_post = 12; %or 10 seconds
...
tag.param_stim.Chance = .5; %likelihood of stimulus occuring
%For trains of pulses
tag.param_stim.Duration_pulse = 20; %milliseconds
...
tag.param_stim.DC_cosine = 1.1;
tag.param_stim.Piezo_Chance = 1;
tag.param_stim.Piezo_Amp = 5;
tag.param_stim.Piezo_Delay = 2;
